...
@Title("Тест выражений с оператором вычитания")
@Description("Проверка результата арифметического действия.")
@RunWith(value = Parameterized.class)
public class SubtractionOperatorParameterizeTest {
    private final int operand1;
    private final int operand2;
    private final String operator;
    private final float expectedResult;
 
    public SubtractionOperatorParameterizeTest(final String A, final String B, final String Oper, final String C) {
    ...
    @Stories("Оператор вычитания")
    @Step("Проверка результата вычислений.")
    @Test
    public void ParameterizeTest() {
        assertEquals(expectedResult, new Arithmetic().SubtractionFunction(operand1,operand2), 0);
    }
}
