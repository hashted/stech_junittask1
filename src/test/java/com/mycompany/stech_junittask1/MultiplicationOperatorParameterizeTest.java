...
@Title("Тест выражений с оператором умножения")
@Description("Проверка результата арифметического действия.")
@RunWith(value = Parameterized.class)
public class MultiplicationOperatorParameterizeTest {
    private final int operand1;
    private final int operand2;
    private final String operator;
    private final float expectedResult;
 
    public MultiplicationOperatorParameterizeTest(final String A, final String B, final String Oper, final String C) {
    ...
    @Stories("Оператор умножения")
    @Step("Проверка результата вычислений.")
    @Test
    public void ParameterizeTest() {
        assertEquals(expectedResult, new Arithmetic().MultiplicationFunction(operand1,operand2), 0);
    }
}